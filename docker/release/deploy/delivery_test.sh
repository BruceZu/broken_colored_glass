...
NAME: delivery docker images.
    ...
    With current commit on 'dev_docker_proj_unsafe' branch to build, tag and delivery images to
...
BUILD_DOCKER_IMAGE_BASH="${MY_PATH}/build_docker_managerapp_images.sh"

[ $REUSE -eq 0 ] &&
    bash "$BUILD_DOCKER_IMAGE_BASH" &&
    release
